...
export default function ResultsList({ candidates, onSelect, selectedId, onFlyToCart }) {
    ...
    return (
        <div className="space-y-2.5">
            <h3 className="px-1 text-[11px] sm:text-xs font-brand font-semibold text-ink-faint uppercase tracking-wider">
            ...
            <div className="grid gap-2">
                {candidates.map((med, index) => (
                    <button
                        ...
                        className={`
                            med-card w-full text-left p-3 sm:p-4 rounded-2xl border-2 transition-all duration-200
                            ${selectedId === med.id
                        ...
                        <div className="flex items-start justify-between">
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                    <span className="text-[15px] sm:text-base font-brand font-bold text-ink-primary leading-snug">
                                        {med.brand_name}
                                    </span>
                                    {med.rx_required ? (
                                ...
                                <p className="text-[13px] sm:text-sm text-ink-muted mt-0.5 font-body">
                                    {med.generic_name} • {med.dosage}
                                </p>

                                {/* Form */}
                                <p className="text-[11px] sm:text-xs text-ink-faint mt-1 capitalize font-body">
                                    {med.form}
                                </p>
                            </div>

                            {/* Stock indicator */}
                            <div className="text-right flex flex-col items-end gap-1 pl-3">
                                {med.stock_quantity > 0 ? (
                                    <span className="feature-badge-emerald text-[11px] sm:text-xs px-2.5 sm:px-3">
